...
export function Article({ title, url, excerpt, image, className }: ArticleProps) {
  ...
  return (
    <article
      className={cn(
        'flex flex-col md:flex-row-reverse border border-gray-90 bg-white shadow-xs text-fg md:p-13 md:gap-xl rounded-lg',
        className
      )}
    >
      <a
        {...externalProps}
        aria-label={title}
        className="group/image relative block w-full md:w-[35%] shrink-0 aspect-[4/3] bg-gray-90 overflow-hidden rounded-md before:absolute before:inset-0 before:z-[1] before:bg-black/20 before:opacity-0 before:transition-opacity before:duration-200 hover:before:opacity-100"
      >
        {image ? (
          ...
          <img
            ...
            alt=""
            loading="lazy"
            className="absolute inset-0 h-full w-full object-coverd"
          />
        ) : null}
      </a>
